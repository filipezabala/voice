...
#' @references Childers DG (1978). Modern Spectrum Analysis. IEEE Computer Society Press.
#'
#' Boersma P. (1993). "Accurate short-term analysis of the fundamental frequency and the harmonics-to-noise ratio of a sampled sound." In Proceedings of the institute of phonetic sciences, volume 17, pp. 97–110. Amsterdam. (\url{http://xeds.eu/other/P_Boersma_ Accurate_short-term_analysis_of_the_fundametnal_freq.pdf})
#'
#' Press W.H., Teukolsky S., Vetterling W.T., Flannery B.P. (1992). Numerical Recipes in C: The Art of Scientific Computing. Cambridge University Press. (\url{https://www.grad.hr/nastava/gs/prg/NumericalRecipesinC.pdf})
#'
#' @examples
#'
#' \dontrun{
...
extract_features_py <- function(directory, filesRange = 0,
  ...
  if('f0_praat' %in% features & !file.exists(paste0(getwd(),'/temp_extract_f0.py'))){
  ...
  if('f0_praat' %in% features){
    extract_f0 <- paste(pycall, './temp_extract_f0.py', directory, windowShift,
    ...
    df_f0 <- df_f0 %>%
      ...
      dplyr::mutate_at(dplyr::vars(-file_name),
                list(as.numeric)) %>%
      dplyr::select(id, file_name, interval, f0_praat) %>%
      dplyr::arrange(file_name, interval)
  }

  # 2. Formants
  if('formants' %in% features){
    extract_formants <- paste(pycall, './temp_extract_formants.py', directory,
